...
import { displayDropdownSelectors, filterSelectors, netflowPage, querySumSelectors } from "@views/netflow-page"
...
describe('(OCP-67087 Network_Observability) DNSTracking test', { tags: ['Network_Observability'] }, function () {
    ...
    it("(OCP-67087, aramesha, Network_Observability) Validate DNSLatencies edge label and Query Summary stats", function () {
        ...
        cy.byTestID('dns_latency').click()
        cy.get(filterSelectors.compareDropdown).should('exist').click().then(() => {
        ...
        cy.get('#metricType').find('li').should('have.length', 3).each((item, index) => {
