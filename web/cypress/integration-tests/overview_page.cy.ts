import { displayDropdownSelectors, genSelectors, netflowPage, overviewSelectors, querySumSelectors } from "@views/netflow-page"
...
describe('(OCP-54839 Network_Observability) Netflow Overview page tests', { tags: ['Network_Observability'] }, function () {
    ...
    it("(OCP-54839, aramesha, Network_Observability) should validate overview page features", function () {
    ...
    it("(OCP-54839, aramesha, Network_Observability) should validate panels", function () {
        cy.showAdvancedOptions().then(views => {
            cy.get(displayDropdownSelectors.overview).should('exist').click()
