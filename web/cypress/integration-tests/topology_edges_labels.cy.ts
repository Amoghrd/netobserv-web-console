import { displayDropdownSelectors, netflowPage, topologySelectors, topologyPage, setupTopologyViewWithNamespaceFilter, getTopologyResourceScopeGroupURL } from "@views/netflow-page"
...
describe("(OCP-53591 Network_Observability) Netflow Topology edges,labels, badges features", { tags: ['Network_Observability'] }, function () {
    ...
    it("(OCP-53591, memodi, Network_Observability) should verify group owners", function () {
        ...
        cy.get(topologySelectors.nGroups).its('length').should('be.gte', 5)
    })
